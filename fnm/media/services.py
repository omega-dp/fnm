...
from fnm.media.models import Media
from fnm.media.utils import (
    bytes_to_mib,
    file_generate_local_upload_url,
    file_generate_name,
    file_generate_upload_path,
)
from fnm.integrations.aws.client import s3_generate_presigned_post
...
def _validate_file_size(file_obj):
    ...
    if file_obj.size > max_size:
        raise ValidationError(f"Media is too large. It should not exceed {bytes_to_mib(max_size)} MiB")
...
class FileStandardUploadService:
    ...
    @transaction.atomic
    def create(self, file_name: str = "", file_type: str = "") -> Media:
    ...
    @transaction.atomic
    def update(self, file: Media, file_name: str = "", file_type: str = "") -> Media:
        _validate_file_size(self.file_obj)
...
class FileDirectUploadService:
    ...
    @transaction.atomic
    def finish(self, *, file: Media) -> Media:
    ...
    @transaction.atomic
    def upload_local(self, *, file: Media, file_obj) -> Media:
